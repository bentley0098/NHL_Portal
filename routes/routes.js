...
const router = app => {
    ...
    app.get('/queryTasks/:UserID&:Priority&:showingClosed&:customer&:department', async (req, res) => {
    ...
    app.put('/updateTask/', async(req, res) => {
        try {
            ...
            .query('UPDATE CRM_ISSUES SET ActionBy = @UserId, Department = @Department, Account = @Account, Company_Name = @Company_Name, Details = @Details, Notes = @Notes, Reference = @Application, Reference2 = @Area, Reference3 = @Contact, Priority = @Priority, Reference4 = @Estimate, Invoice = @Invoice, DueDate = @DueDate, Requested = @Requested WHERE Issue_No = @taskID', (error, result) => {
                if (error) throw error
    ...
    app.post('/addNewTask/', async(req, res) => {
        try {
            ...
            .query(`INSERT INTO CRM_ISSUES ( Owner_Name, Issue_No, State, Account, Company_Name, Reference, Reference2, Reference3, Reference4 , Product_Code, Details, Department, Requested, Time, Complete , Notes,Priority, ActionBy, DueDate, Invoice, Urgent) VALUES ( @Owner, @taskID, 'O', @CustomerCode, @CustomerName, @Ref1, @Ref2, @Contact, @Estimate, '', @Details, @Department, @ReqDate, '0', '0', @Notes, @Priority, @UserID, @DueDate, @Invoice, @Urgent)`, (error, result) => {
                if (error) throw error
    ...
    app.get('/getTimeSpent/:startDate&:endDate&:user', async(req,res) => {
        //var queryString = "WEB_LOADQUICKSUPPORTSTATS @From='" + req.params.startDate+  "', @To='" + req.params.endDate+"'";
        
        var queryString = "RPT_TIME_BY_CUSTOMER @DateFrom='"+req.params.startDate+"', @DateTo='"+req.params.endDate + "', @UserID=" + req.params.user;
        //if(req.params.user!=='none') {
        //    queryString= queryString + ", @ActionBy=" + req.params.user;
        //}
        //console.log(queryString)
    ...
    app.get('/returnErrors/:code', async (req, res) => {
        try {
            ...
            .query(` WEB_RETURNERRORS @CustomerCode= ` + req.params.code + ` ` )      
